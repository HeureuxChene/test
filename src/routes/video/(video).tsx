...
export default function Video() {
    ...
    const [filtreTexte] = createSignal(['TOUT VOIR', 'MOTION DESIGN', 'EFFETS SPÉCIAUX', 'INCRUSTATION', 'SOUND DESIGN']);
    ...
    return (
        <>
            <section id='homeRef' class={style.homeRef}>
                <video id={style.homeRefVideo} autoplay muted loop playsinline preload='auto' src={`/media/video/ref-video/video-accueil.mp4`} />
                ...
                <A href='/design' class={`${style.linkCatRef} ${style.linkLeft}`}>
                    <svg id={style.linkDesignFleche} height="20" width="20" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden="true">
                        <path d="m5.52 0 7.91 7.5-7.91 7.5-1.95-1.95 5.83-5.55-5.83-5.55z" />
                    </svg>
                    Design
                </A>
                ...
                <section id={style.referencesSection} style={{ height: `calc(${Math.ceil(numberOfVisibleBrique(briqueFiltre()) / (tabletSize() === 1 ? 1 : 3))} * (100vw / ${(tabletSize() === 1 ? 1 : 3)}))` }}>
                    {
                        Object.keys(media).map((key, index) => {
                            return (
                                <A href={`/video/${key}`} class={`${style.referencesLink} ${briqueFiltre()[index] ? style.visible : style.invisible}`}>
                                    ...
                                    <img class={style.referencesImg} alt={key} src={`/media/photo/ref-video/${key}/${(media as any)[key]['vignette']}`} width={1080} height={846} />
                                </A>
                            )
                        })
                    }
